...
def caesar_cipher(str, shift)
  ...
  shifted_words = words.map do |word|
    arr_shift = word.chars.map do |letter|
      ind = chars.find_index(letter)
      chars[(ind+shift)%chars.length]
    end
    arr_shift.join("")
  end
  shifted_words.join(" ")
...
def jumble_sort(str, alphabet = nil)
  ...
  sorted = str.chars.sort_by { |char| alphabet.find_index(char) }
  sorted.join("")
end
...
class String
  ...
  def real_words_in_string(dictionary)
    ...
    dictionary.each do |word|
      real_words << word if self.include?(word) && !real_words.include?(word)
    end
    real_words
  end
end
